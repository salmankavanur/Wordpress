...
define( 'WP_2FA_DIGIBAYT_VERSION', '1.0.0' );
define( 'WP_2FA_DIGIBAYT_PATH', plugin_dir_path( __FILE__ ) );
define( 'WP_2FA_DIGIBAYT_URL', plugin_dir_url( __FILE__ ) );
...
foreach ( $wp_2fa_core_files as $file ) {
    $path = WP_2FA_DIGIBAYT_PATH . $file;
	if ( file_exists( $path ) ) {
		require_once $path;
	}
}
...
add_action( 'init', function() {
	if ( class_exists( 'WP_2FA_DigiBayt_Auth' ) ) {
		new WP_2FA_DigiBayt_Auth();
	}
	if ( class_exists( 'WP_2FA_DigiBayt_REST' ) ) {
		new WP_2FA_DigiBayt_REST();
	}
} );

// 4. Admin Menu Registration
add_action( 'admin_menu', 'wp_2fa_digibayt_register_menu', 10 );
function wp_2fa_digibayt_register_menu() {
...
// 5. Admin Render
function wp_2fa_digibayt_render_admin() {
	echo '<div class="wrap">';
	echo '<div id="wp-2fa-digibayt-admin"><p>' . __( 'Loading Security Dashboard...', 'wp-2fa-digibayt' ) . '</p></div>';
	echo '</div>';
}

// 6. Enqueue Scripts
add_action( 'admin_enqueue_scripts', 'wp_2fa_digibayt_enqueue_assets' );
function wp_2fa_digibayt_enqueue_assets( $hook ) {
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_enqueue_style(
			'wp-2fa-digibayt-admin',
			WP_2FA_DIGIBAYT_URL . 'build/style-index.css',
			array(),
			$assets['version']
		);
	}
...
add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'wp_2fa_digibayt_action_links' );
function wp_2fa_digibayt_action_links( $links ) {
	$settings_link = '<a href="' . admin_url( 'admin.php?page=wp-2fa-auth' ) . '">' . __( 'Settings', 'wp-2fa-digibayt' ) . '</a>';
	array_unshift( $links, $settings_link );
	return $links;
}

// 8. Activation
register_activation_hook( __FILE__, 'wp_2fa_digibayt_activate' );
function wp_2fa_digibayt_activate() {
	global $wpdb;
