...
class WP_2FA_Auth_DigiBayt_REST {
	...
	public function verify_totp( $request ) {
		...
		$secret = get_user_meta( $user_id, '_wp_2fa_auth_digibayt_totp_pending_secret', true );
		...
		if ( $totp->verify_code( $secret, $code ) ) {
			update_user_meta( $user_id, '_wp_2fa_auth_digibayt_totp_secret', $secret );
			update_user_meta( $user_id, '_wp_2fa_auth_digibayt_enabled', 'yes' );
			delete_user_meta( $user_id, '_wp_2fa_auth_digibayt_totp_pending_secret' );
	...
	public function check_admin_permission() {
		return current_user_can( WP_2FA_AUTH_DIGIBAYT_CAPABILITY );
	}
	...
	public function check_logs_permission() {
		return current_user_can( WP_2FA_AUTH_DIGIBAYT_LOGS_CAPABILITY );
	}
	...
	public function update_settings( $request ) {
		$params = $request->get_json_params();
		update_option( 'wp_2fa_auth_digibayt_settings', $params );
		return rest_ensure_response( array( 'success' => true ) );
	}
	...
	public function get_user_config() {
		$user_id = get_current_user_id();
		$config = array(
			'enabled' => get_user_meta( $user_id, '_wp_2fa_auth_digibayt_enabled', true ) === 'yes',
			'methods' => get_user_meta( $user_id, '_wp_2fa_auth_digibayt_methods', true ),
		);
