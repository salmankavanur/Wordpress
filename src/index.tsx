...
const startApp = () => {
	...
	const container = document.getElementById( 'wp-2fa-auth-digibayt-admin' );
	...
	try {
		...
		if ( window.wp && ! window.wp.components )
			missing.push( 'wp.components' );

		if ( missing.length > 0 ) {
			container.innerHTML =
				'Error: Missing WordPress packages: ' + missing.join( ', ' );
		...
		container.innerHTML =
			'Error starting Security Dashboard. Please check browser console (F12).';
	}
};
