...
const Settings: React.FC = () => {
	...
	const saveSettings = () => {
		setIsSaving(true);
		apiFetch({
			path: '/wp-2fa-digibayt/v1/settings',
			method: 'POST',
			data: settings,
		}).then(() => {
	...
	return (
		<div className="tab-content">
			...
			{notice && <Notice status="success" onRemove={() => setNotice(null)}>{notice}</Notice>}
			...
			<div className="settings-form">
				<ToggleControl
					...
					onChange={(val) => setSettings({ ...settings, enforce_admins: val })}
				/>
				...
				<SelectControl
					...
					onChange={(val) => setSettings({ ...settings, grace_period: parseInt(val) })}
				/>
				...
				<SelectControl
					...
					onChange={(val) => setSettings({ ...settings, remember_device: parseInt(val) })}
				/>

				<Button isPrimary busy={isSaving} onClick={saveSettings}>
					Save Settings
				</Button>
			</div>
		</div>
