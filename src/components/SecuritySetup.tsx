...
const SecuritySetup: React.FC = () => {
	...
	const fetchConfig = () => {
		...
		apiFetch( {
			path: `/wp-2fa-auth-digibayt/v1/user/config?_=${ Date.now() }`,
		} ).then( ( res: any ) => {
	...
	const startTotpSetup = () => {
		apiFetch( {
			path: `/wp-2fa-auth-digibayt/v1/user/totp/setup?_=${ Date.now() }`,
		} ).then( ( res: any ) => {
			setTotpSetup( res );
			setIsModalOpen( true );
		} );
	...
	const verifyTotp = () => {
		apiFetch( {
			path: '/wp-2fa-auth-digibayt/v1/user/totp/verify',
			method: 'POST',
	...
	const generateBackupCodes = () => {
		apiFetch( {
			path: '/wp-2fa-auth-digibayt/v1/user/backup-codes/generate',
			method: 'POST',
		} ).then( ( res: any ) => {
